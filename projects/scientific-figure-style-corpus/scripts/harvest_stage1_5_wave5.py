...
def load_hashes(root,current):
 hashes={}
 for p in [root/'manifests/stage1_5_harvested_figures.csv',root/'manifests/stage1_5_wave2_harvested_figures.csv',root/'manifests/stage1_5_wave3_harvested_figures.csv',root/'manifests/stage1_5_wave4_harvested_figures.csv']:
  if not p.exists():continue
  for r in csv.DictReader(p.open(encoding='utf-8',newline='')):
   h=r.get('sha256','').strip()
   if h:hashes.setdefault(h,[]).append(str(p.name)+':'+r.get('candidate_id','')+':fig'+r.get('figure_number',''))
 return hashes
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--project-root',default='projects/scientific-figure-style-corpus');ap.add_argument('--selection',default='projects/scientific-figure-style-corpus/annotations/stage1_5_wave5_figure_screening.csv');a=ap.parse_args()
 root=Path(a.project_root).resolve();sel=Path(a.selection).resolve();assets=root/'assets/stage1_5';manifest=root/'manifests/stage1_5_wave5_harvested_figures.csv';report=root/'validation/stage1_5_wave5_harvest_report.json';status=root/'validation/stage1_5_status.json';finalv=root/'validation/stage1_5_wave5_final_validation.json';assets.mkdir(parents=True,exist_ok=True);manifest.parent.mkdir(parents=True,exist_ok=True);report.parent.mkdir(parents=True,exist_ok=True)
 rows=list(csv.DictReader(sel.open(encoding='utf-8',newline='')));selected=[r for r in rows if truthy(r['public_mirror'])];s=requests.Session();s.headers.update({'User-Agent':'codex-workbench-stage1.5-wave5/0.1','Accept-Language':'en-US,en;q=0.8'});out=[];fail=[];cache={}
 for r in selected:
 ...
 hashes=[x['sha256'] for x in out];within=sorted({x for x in hashes if hashes.count(x)>1});prior=load_hashes(root,manifest);cross=[{'sha256':x['sha256'],'wave5':x['candidate_id']+':fig'+x['figure_number'],'prior':prior[x['sha256']]} for x in out if x['sha256'] in prior]
 file_errors=[]
 for x in out:
  p=root/x['asset_path']
 ...
 rep={'wave':5,'screened_figure_rows':len(rows),'public_mirror_selected':len(selected),'harvested_assets':len(out),'failed_or_rejected':len(fail),'tier_counts':dict(Counter(x['style_learning_tier'] for x in out)),'within_wave_duplicate_hashes':within,'cross_wave_duplicates':cross,'file_validation_errors':file_errors,'failures':fail};report.write_text(json.dumps(rep,indent=2,ensure_ascii=False),encoding='utf-8')
 w1=active(root/'manifests/stage1_5_harvested_figures.csv');w2=active(root/'manifests/stage1_5_wave2_harvested_figures.csv');w3=active(root/'manifests/stage1_5_wave3_harvested_figures.csv');w4=active(root/'manifests/stage1_5_wave4_harvested_figures.csv');w5=active(manifest);total=42+w1+w2+w3+w4+w5
 st={'stage':'1.5_recent_targeted_refinement','stage1_ab_baseline':42,'wave1_active_ab_added':w1,'wave2_active_ab_added':w2,'wave3_active_ab_added':w3,'wave4_active_ab_added':w4,'wave5_active_ab_added':w5,'current_ab_after_wave5':total,'target_ab_range':[80,100],'wave5_screened_figures':len(rows),'wave5_public_assets':len(out),'wave5_validation':'passed' if ok else 'failed','next_action':'active A+B target reached; perform corpus-level balance QA' if total>=80 and ok else 'repair Wave 5 validation or continue targeted refinement'};status.write_text(json.dumps(st,indent=2),encoding='utf-8');finalv.write_text(json.dumps({'validated_assets':len(out),'selected_assets':len(selected),'checksum_validation':'passed' if not file_errors else 'failed','within_wave_duplicate_hashes':within,'cross_wave_duplicates':cross,'manifest_complete':len(out)==len(selected),'validation_passed':ok,'active_ab_total':total},indent=2),encoding='utf-8');print(json.dumps(rep,indent=2));print(json.dumps(st,indent=2));return 0 if ok and out else 2
