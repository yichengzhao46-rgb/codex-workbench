...
POSITIVE={"schematic":6,"mechanism":6,"model":3,"pathway":4,"electron transfer":8,"extracellular electron transfer":10,"direct interspecies electron":10,"interface":8,"material":4,"mineral":7,"iron oxide":6,"biofilm":5,"electrode":5,"redox":6,"conductive":7,"cytochrome":6,"nanowire":6,"mtrcab":8,"methane":4,"methanotroph":6,"gradient":6,"oxic":3,"anoxic":4,"hypoxic":4,"semiconductor":7,"biohybrid":7,"photoelectron":8,"conceptual":5,"overview":4,"workflow":2}
NEGATIVE={"cross-feeding":-2,"heatmap":-5,"principal component":-5,"pca":-5,"volcano":-5,"boxplot":-4,"box plot":-4,"western blot":-4,"microscopy":-3,"micrograph":-3,"sem image":-3,"tem image":-3,"phylogen":-4,"bar chart":-2}
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--project-root',default='projects/scientific-figure-style-corpus');ap.add_argument('--candidates',default='projects/scientific-figure-style-corpus/manifests/stage1_5_wave6_candidates.csv');a=ap.parse_args()
 root=Path(a.project_root).resolve();scan_dir=root/'validation/stage1_5_wave6_scan';previews=scan_dir/'previews';contacts=scan_dir/'contact_sheets';previews.mkdir(parents=True,exist_ok=True);contacts.mkdir(parents=True,exist_ok=True)
 rows=list(csv.DictReader(Path(a.candidates).open(encoding='utf-8',newline='')));s=requests.Session();s.headers.update({'User-Agent':'codex-workbench-stage1.5-wave6/0.1','Accept-Language':'en-US,en;q=0.8'});scanned=[];summary={'articles_total':len(rows),'articles_scanned':0,'figures_scanned':0,'previews_resolved':0,'public_mirror_compatible_articles':0,'restrictive_or_unknown_articles':0,'article_failures':[]}
 for row in rows:
 ...
 out_csv=scan_dir/'stage1_5_wave6_all_figures.csv';scan_dir.mkdir(parents=True,exist_ok=True)
 with out_csv.open('w',encoding='utf-8',newline='') as f:w=csv.DictWriter(f,fieldnames=FIELDS);w.writeheader();w.writerows(scanned)
 (scan_dir/'stage1_5_wave6_scan_summary.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False),encoding='utf-8');print(json.dumps(summary,indent=2,ensure_ascii=False));return 0 if summary['articles_scanned'] else 2
